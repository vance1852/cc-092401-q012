    ...
    def handle(
        ...
        try:
            ...
            if method == "POST" and path == "/comparisons":
                result = self.service.compare_batches(
                    self._actor(normalized_headers), payload["baseline_batch_id"],
                    payload["candidate_batch_id"], int(payload["baseline_analysis_id"]),
                    int(payload["candidate_analysis_id"]), {"rules": payload.get("rules", [])},
                )
                return Response(201 if result["created"] else 200, result)
