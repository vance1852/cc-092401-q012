...
- `service.py`：角色权限、批次状态机、幂等导入、排除复核、任务租约、审批、对照和报告；
- `analysis.py`：分层覆盖、Wilson 区间、描述性统计、确定性 bootstrap 和准入规则；
- `comparison.py`：批次间对照、Newcombe 比例差区间、合并效应量和确定性 bootstrap 差值区间；
- `contracts.py`：协议、指标、分层、权重、随机种子、对照规则和单次观测的数据契约；
- `jsonio.py`：严格 JSON/JSONL 读取、规范化序列化与内容摘要；
- `numeric.py`：不依赖第三方库的描述性统计、Wilson 与 Newcombe 区间；
...
构建对照分析用于判断候选构建相对已准入基线改善还是退化：统计负责人可以选择两个已封存且协议版本、机器人型号和预注册分层兼容的批次，系统冻结双方当前分析版本的输入摘要并复核排除未再变化；二元指标给出成功率差及 Newcombe 置信区间，连续与计数指标给出均值差、合并标准差效应量和确定性 bootstrap 区间，再依据预先声明的非劣或优效规则形成逐项结论。协议或型号不兼容、引用了非当前分析版本、分析后排除发生变化时拒绝建立对照；任一预注册分层缺样时形成明确的不足结论。对照结果按算法版本固化、可按相同输入幂等重算并进入两个批次的报告，但不会改变两个原批次的准入决定。
...
下面的命令会读取 `fixtures/` 中的协议与观测记录，在临时 SQLite 数据库中完成用户与设备建档、协议发布、批次启动、观测导入、批次封存、任务领取、统计分析、准入审批、两个构建批次的对照分析和审计报告导出，随后输出一行 JSON 结果：
